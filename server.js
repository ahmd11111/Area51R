...
async function sendDiscordWebhook(order) {
  ...
  const embed = {
    embeds: [{
      ...
      footer: { text: 'X5 Store - Automated Delivery System' }
    }]
  };
...
const db = new Low(adapter, {
  ...
  siteSettings: {
    ...
    heroTitle: 'Welcome to X5',
    heroSubtitle: 'Discover amazing products at unbeatable prices',
    rulesContent: 'Welcome to our store! Please read our rules before shopping.',
    supportEmail: 'support@x5.com',
...
// Start server
app.listen(PORT, () => {
  console.log(`X5 Store server running on http://localhost:${PORT}`);
});